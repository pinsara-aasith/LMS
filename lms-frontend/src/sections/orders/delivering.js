...
import { getOrdersCancelled, getOrdersDelivering, getOrdersStillInWareHouse } from 'src/apis/orders';
import { getAllTransportationDelivery } from 'src/apis/transportation_train_trips';
...
export const OrdersDeliveringPanel = () => {
        ...
        <Box sx={{ marginLeft: '40px', borderBottom: 1, borderColor: 'divider' }}>
          ...
          stores?.map(s => (
            <TabPanel key={s.Id} value={`${s.Id}`}>
              <OrdersDeliveringForStore key={s.Id} storeId={s.Id} />
            </TabPanel>
          ))
        }
      </TabContext>
...
export const OrdersDeliveringForStore = ({ storeId }) => {
  const [loading, setLoading] = useState(false);
  ...
  async function retrieveAndRefreshData() {
    setLoading(true)
    try {
      const orders = (await getOrdersDelivering(storeId)) || [];
      setData(orders)
    } catch (e) {
      enqueueSnackbar('Error while doing network operations...', {
        variant: 'error',
  ...
  const handleDelivering = async (orderId) => {
    confirm({ description: `Are you sure delivering the order?` })
      .then(async () => {
        try {
          setLoading(true)
